...
The only user-wide administrative routes are `POST /users` and `GET /users`. Project members use `GET /projects/{project_id}/eligible-users` to select registered users for their own project.
